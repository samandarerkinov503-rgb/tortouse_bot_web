...
function updateQuantity(productId, change) {
    if (!cart[productId]) {
        cart[productId] = { id: productId, qty: 0, type: "product", name_uz: products.find(p => p.id === productId).name_uz, name_ru: products.find(p => p.id === productId).name_ru, price: products.find(p => p.id === productId).price };
    }
